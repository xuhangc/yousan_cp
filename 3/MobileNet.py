...
if __name__ == '__main__':
    ...
    # Example input (batch size of 1 with 3 color channels and 224x224 image size)
    example_input = torch.randn(1, 3, 224, 224)
    example_label = torch.tensor([1])  # Example label

    # Forward pass
    pred = model(example_input)
    print(pred.shape)

    loss = F.cross_entropy(pred, example_label)
    acc = (pred.argmax(dim=1) == example_label).float().mean()